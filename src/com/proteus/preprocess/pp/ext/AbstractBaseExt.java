...
import com.proteus.preprocess.pp.GetFormattedText;
import com.proteus.preprocess.pp.PopulateExtendedContextVisitor;
import com.proteus.preprocess.pp.SetExtendedContextVisitor;
...
public abstract class AbstractBaseExt implements GetFormattedText{
	...
	protected PPParser getPrimeParser(String str){
		PPLexer lexer = new PPLexer(new ANTLRInputStream(str));
		CommonTokenStream tokens = new CommonTokenStream(lexer);
		return new PPParser(tokens);
	}
		...
		if(ctx != null && ctx.children != null && ctx.children.size()>0){
			for(ParseTree childCtx : ctx.children){
				if(!(childCtx instanceof TerminalNode) && childCtx.getText().length() >0){
					...
					if ( textContext != null) return textContext;
				}
			}
		}
		return null;
	...
	public void processModuleInstantiation(PreprocessRunnerSession proteusRunnerSession) {
		ParserRuleContext ctx = getContext();
		if(ctx != null && ctx.children != null && ctx.children.size()>0){
			for(ParseTree childCtx : ctx.children){
				if(!(childCtx instanceof TerminalNode) && childCtx.getText().length() >0){
					getExtendedContextVisitor().visit(childCtx).processModuleInstantiation(proteusRunnerSession);
				}
			}
		} 
	}
