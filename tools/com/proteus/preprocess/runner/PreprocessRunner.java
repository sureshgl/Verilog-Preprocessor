...
public class PreprocessRunner {
	...
	public static void runVerilogParserRunner(String[] args, boolean systemExitOnException){
		...
		try{
			...
			PreprocessRunnerSession session = new PreprocessRunnerSession(cp.getOutputFile(), cp.getInputFilesInOrder() , cp.getYOptionFiles(), cp.getMacroDefinitions(), null,null,filePreprocessStatusMap,null);
			session.preprocess();
			L.info("Done");
		}catch(Throwable t){
			t.printStackTrace();
