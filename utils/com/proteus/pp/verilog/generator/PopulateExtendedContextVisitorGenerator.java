package com.proteus.pp.verilog.generator;
...
public class PopulateExtendedContextVisitorGenerator {
	...
	private static void generateTestClasses(Map<String, RuleType> rules) throws FileNotFoundException {
		...
		sb.append("package com.proteus.preprocess.pp;\n\n");

		sb.append("import org.antlr.v4.runtime.ParserRuleContext;\n\n");
		sb.append("import com.proteus.preprocess.pp.ext.*;\n");
		sb.append("import com.proteus.preprocess.pp.gen.PPParser;\n");
		sb.append("import com.proteus.preprocess.pp.gen.PPParserBaseVisitor;\n");

		sb.append("public class PopulateExtendedContextVisitor extends PPParserBaseVisitor<ParserRuleContext>{\n");
		for(Entry<String, RuleType> e:rules.entrySet()){
			if(! e.getValue().isAlt()){
				count++;
				String newName = e.getKey().substring(0, 1).toUpperCase() + e.getKey().substring(1);
				sb.append("	@Override public org.antlr.v4.runtime.ParserRuleContext visit"+newName+"(PPParser."+newName+"Context ctx) {\n");
		...
		sb.append("}");
		FileUtils.WriteFile(new File("tools/com.proteus.preprocess/pp/PopulateExtendedContextVisitor.java"), sb);

		System.out.println("Generated "+ count + " classes");
	}

	public static void main(String[] args) throws IOException {
		generateTestClasses(Antlr4Reflection.getParserRules("grammar/PPParser.g4"));
	}
