...
public class ExtendedContextVisitorGenerator {
	...
	private static void generateTestClasses(Map<String, RuleType> rules) throws FileNotFoundException {
		...
		sb.append("public class ExtendedContextVisitor extends PPParserBaseVisitor<AbstractBaseExt>{\n");
		for(Entry<String, RuleType> e:rules.entrySet()){
			if(! e.getValue().isAlt()){
				count++;
				String newName = e.getKey().substring(0, 1).toUpperCase() + e.getKey().substring(1);
				sb.append("	@Override public AbstractBaseExt visit"+newName+"(PPParser."+newName+"Context ctx) {\n");
		...
		sb.append("}");
		FileUtils.WriteFile(new File("tools/com.proteus.preprocess/pp/ExtendedContextVisitor.java"), sb);
